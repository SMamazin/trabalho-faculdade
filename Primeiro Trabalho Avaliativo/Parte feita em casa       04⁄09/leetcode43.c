...
char *multiply( char *num1, char *num2 ) {
	...
	int maxVarCima = strlen( num1 );
	int maxVarBaixo = strlen( num2 );	

	int* final = ( int* ) calloc (( maxVarCima + maxVarBaixo ), sizeof( int ));
	int calc = 0;

	if ( num1 != NULL && num2 != NULL ) {
		for ( int i = maxVarBaixo-1; i >= 0; i-- ) {
			for ( int j = maxVarCima-1; j >= 0; j-- ) {
				int cima = num1[j] - 48;
				int baixo = num2[i] - 48;
				
				calc = ( cima * baixo ) + final[ i + j + 1 ];
	...
	while ( start < ( maxVarCima + maxVarBaixo ) && final[start] == 0) {
		start++;
	}

	if ( start == ( maxVarCima + maxVarBaixo ) ) {
	...
	int tamanho = ( maxVarCima + maxVarBaixo ) - start;
	char* res = ( char* ) malloc ( ( tamanho + 1 ) + sizeof( char ) );
	
	for ( int a = 0; a < tamanho; a++ ){
		res[a] = final[start + a] + 48;
	}
	res[tamanho] = '\0';
